...
const ListGroup = ({ data }) => {
  console.log(data);
	...
	const side = data.filter((item) => item.category.part === title[0]);
	const soup = data.filter((item) => item.category.part === title[1]);
	const dessert = data.filter((item) => item.category.part === title[2]);
	const special = data.filter((item) => item.category.part === title[3]);
	return (
		<>
			<Title title={title[0]} />
